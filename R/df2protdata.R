...
#' @param int_cols A vector of character strings or numeric indices indicating which column(s) in the data frame \code{df} contain peptide intensities. If \code{int_cols} contains only one element, \code{df2protdata} will assume that data frame \code{df} is in "long" format. If \code{int_cols} contains more than one element, \code{df2 protdata} will assume the data to be in "wide" format.
#' @param int_name A character string indicating the name that will be given to the column containing the intensity values. Defaults to "quant_value".
#' @param run_name If int_cols contains more than one element (i.e. the data is in "wide" format), this should contain a freely chosen character string indicating the name that will be given to the column containing the mass spec run names. If no name is chosen (\code{run_name=NULL}), this will default to \code{"run"}.
#' If int_cols contains only one element (i.e. the data is in "long" format), \code{run_name} should contain the name of the column that contains the run names.
...
#' int_cols <- colnames(pepdf)[which(grepl("Intensity.",colnames(pepdf)))]
#' #Log2 transform data and change -Inf to NA
#' pepdf[,int_cols] <- log2(pepdf[,int_cols])
#' tmp_ints <- pepdf[,int_cols]
#' tmp_ints <- as.matrix(tmp_ints)
#' tmp_ints[is.infinite(tmp_ints)] <- NA
#' tmp_ints <- as.data.frame(tmp_ints)
#' pepdf[,int_cols] <- tmp_ints
#' #Keep only columns of interest
#' pepdf <- pepdf[,c(int_cols,"Proteins","Sequence","PEP")]
#' #Dermine the column that contains the protein names
#' acc_col <- "Proteins"
#' proteins <- df2protdata(pepdf, acc_col, int_cols)
#' @include protdata.R
#' @export
df2protdata <- function(df, acc_col, int_cols, int_name="quant_value", run_name=NULL, annotations=NULL){

  if(is.null(run_name) & length(int_cols)==1){stop("Please provide the name of the column containing the mass spec runs as input in run_name.")}
  if(is.null(run_name) & length(int_cols)>1){run_name <- "run"}
  ...
  for(i in 1:length(proteins)){
    ...
    intensities <- df[sel,int_cols, drop=FALSE]
    ...
    properties <- properties[,-which(colnames(properties) %in% c(colnames(df[,acc_col,drop=FALSE]),colnames(df[,annotations,drop=FALSE]),colnames(df[,int_cols,drop=FALSE]))), drop=FALSE]
