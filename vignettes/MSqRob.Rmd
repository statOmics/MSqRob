...
Note that an "X" is prepended to the sample names because a syntactically valid name in R cannot start with a number.
...
phenotype <- factor(substr(runs, 1,4))
```
...
```{r eval=FALSE}
exp_annotation <- data.frame(run=runs, phenotype=phenotype, biorep=biorep)
exp_annotation
```

Note that here, there is no "X" prepended. However, names will be changed automatically to syntactically valid names by MSqRob during preprocessing.
...
```{r eval=FALSE}
file_exp_annotation <- system.file("extdata/Francisella", "label-free_Francisella_annotation.xlsx", package = "MSqRob")
file_exp_annotation
```
...
```{r eval=FALSE}
#Import Excel file
exp_annotation2 <- openxlsx::read.xlsx(file_exp_annotation)
#Convert columns of data frame to factors
exp_annotation2 <- as.data.frame(unclass(exp_annotation2))
exp_annotation2
```
...
```{r eval=FALSE}
system.time(protLMFranc <- fit.model(proteinsFranc, response="quant_value", fixed=c("phenotype"),  random=c("biorep","run","Sequence"), add.intercept=TRUE, weights="Huber"))
```
...
Here, we notice that the mutant is encoded as "phenotypeX3D8" and the wild type as "phenotypeX1WT". We construct a contrast matrix L for inferring on the fold change of interest. We are interested in the log2 fold change between mutant and wild type. As all intensities are log2-transformed, we should take the difference between "phenotypeX3D8" and "phenotypeX1WT" (because of the following property of logarithms: log2(a/b)=log2(a)-log2(b)).
```{r eval=FALSE}
L <- makeContrast(contrasts=c("phenotypeX3D8-phenotypeX1WT"),
                  levels=c("phenotypeX1WT","phenotypeX3D8"))
L
```
...
```{r eval=FALSE}
#Keep eval=FALSE
L <- makeContrast(contrasts=c("phenotypeX3D8/2+phenotypeX1WT/2"),
                  levels=c("phenotypeX1WT","phenotypeX3D8"))
L
```

**Important notification:** do not use the estimated parameters beta to determine the factor levels! You will notice for example that the level "phenotypeX1WT" is missing in the column names of the betas. This is because "phenotypeX1WT" is the reference class, giving the parameter corresponding to "phenotypeX3D8" the interpretation of the effect of "phenotypeX3D8" relative to "phenotypeX1WT". It is important however to include "phenotypeX1WT" in the contrast matrix because for proteins where factor levels are missing, these parameters can get different interpretations. The `makeContrast` function takes this into account and will return `NA` when a contrast cannot be properly estimated.
...
```{r eval=FALSE}
condition <- factor(substr(runs, 1,2))
```
...
```{r eval=FALSE}
lab <- factor((as.numeric(substr(runs, 4,4))-1)%/%3+1)
```
...
```{r eval=FALSE}
system.time(modelRRCPTAC <- fit.model(protdata=proteinsCPTAC, response="quant_value", fixed="condition", random=c("Sequence","lab"), add.intercept=TRUE, shrinkage.fixed=NULL, weights="Huber", k = 1.345, tolPwrss = 1e-10, verbose=FALSE))
```

Fit a ridge model without M estimation with Huber weights.
```{r eval=FALSE}
system.time(modelRidgeCPTAC <- fit.model(protdata=proteinsCPTAC, response="quant_value", fixed="condition", random=c("Sequence","lab"), add.intercept=TRUE, shrinkage.fixed=NULL, weights=NULL, k = 1.345, tolPwrss = 1e-10, verbose=FALSE))
```

Fit an OLS model with M estimation with Huber weights.
```{r eval=FALSE}
system.time(modelLmMCPTAC <- fit.model(protdata=proteinsCPTAC, response="quant_value", fixed=c("condition","Sequence","lab"), random=NULL, add.intercept=TRUE, shrinkage.fixed=c(0,0,0), weights="Huber", k = 1.345, tolPwrss = 1e-10, verbose=FALSE))
```

Fit an OLS model without M estimation with Huber weights.
```{r eval=FALSE}
system.time(modelLmCPTAC <- fit.model(protdata=proteinsCPTAC, response="quant_value", fixed=c("condition","Sequence","lab"), random=NULL, add.intercept=TRUE, shrinkage.fixed=c(0,0,0), weights=NULL, k = 1.345, tolPwrss = 1e-10, verbose=FALSE))
```
...
lm_normal <- test.protLMcontrast(modelLmCPTAC, L, simplify=TRUE)
...
lm_Sq <- test.protLMcontrast(modelLmCPTAC, L, simplify=TRUE)
...
lm_SqM <- test.protLMcontrast(modelLmMCPTAC, L, simplify=TRUE)
...
ridge <- test.protLMcontrast(modelRidgeCPTAC, L, simplify=TRUE)
...
ridgeSq <- test.protLMcontrast(modelRidgeCPTAC, L, simplify=TRUE)
...
ridgeSqM <- test.protLMcontrast(modelRRCPTAC, L, simplify=TRUE)
...
```{r eval=FALSE}
custom_dfs <- sapply(data, function(x){return(length(unique(paste0(x$condition,x$lab)))-length(unique(x$condition))-length(unique(x$lab))+2-1)})
custom_dfs
