...
phenotype <- factor(substr(runs, 1,3))
```
...
Then, import the data using the `read.xlsx` function fromt the `openxlsx` package and convert the columns of the data frame to factors.
```{r eval=FALSE}
#Import Excel file
...
Here, we notice that the mutant is encoded as "phenotype3D8" and the wild type as "phenotype1WT". We construct a contrast matrix L for inferring on the fold change of interest. We are interested in the log2 fold change between mutant and wild type. As all intensities are log2-transformed, we should take the difference between "phenotype3D8" and "phenotype1WT" (because of the following property of logarithms: log2(a/b)=log2(a)-log2(b)).
```{r eval=FALSE}
L <- makeContrast(contrasts=c("phenotype3D8-phenotype1WT"),
                  levels=c("phenotype1WT","phenotype3D8"))
L
```
...
```{r eval=FALSE}
#Keep eval=FALSE
L <- makeContrast(contrasts=c("phenotype3D8/2+phenotype1WT/2"),
                  levels=c("phenotype1WT","phenotype3D8"))
L
```

**Important notification:** do not use the estimated parameters beta to determine the factor levels! You will notice for example that the level "phenotype1WT" is missing in the column names of the betas. This is because "phenotype1WT" is the reference class, giving the parameter corresponding to "phenotype3D8" the interpretation of the effect of "phenotype3D8" relative to "phenotype1WT". It is important however to include "phenotype1WT" in the contrast matrix because for proteins where factor levels are missing, these parameters can get different interpretations. The `makeContrast` function takes this into account and will return `NA` when a contrast cannot be properly estimated.
