...
```{r eval=FALSE}
peptidesFranc <- peptidesFranc[,c(1:9,19:27)]
```
...
```{r eval=FALSE}
system.time(protLMFranc <- fit.model(proteinsFranc, response="quant_value", fixed=c("pheno"),  random=c("biorep","techrep","Sequence"), add.intercept=TRUE, weights="Huber"))
```
...
The `acc_col` argument in the `df2protdata` function indicates the column index or column name of the accessions (i.e. the protein (group) identifiers). The `int_cols` argument is a vector of indices pointing to all columns containing peptide intensities. In the example below, the column named "Proteins" will be added to the `accession` slot while all columns with indices in the `int_cols` vector will be added as a matrix to the `intensities` slot. `int_name` indicates the name that will be given to the column containing the (preprocessed) intensities and `run_name` indicates the name that will be given to the column containing the mass spec run names in the new `protdata` object. Like before, we keep "GI_number" and "Protein.names" as annotations.

```{r eval=FALSE}
#Select columns with intensities.
int_cols <- which(grepl("Intensity.", colnames(pepFrancDf2)))
int_cols
```
...
proteinsFrancDf <- df2protdata(pepFrancDf2, acc_col="Proteins", int_cols=int_cols, int_name="quant_value", run_name="run", annotations=c("GI_number","Protein.names"))
...
Extract the names of the mass spec runs.
```{r eval=FALSE}
...
You will be alble to load the data from the future MsqRobData package for comparison.
```{r eval=FALSE}
...
Advanced: check which models would fail to converge by building a linear mixed effects model.
```{r eval=FALSE}
convergence <- lapply(data, function(x){return(tryCatch(lme4::lFormula(formula("value~1+(1|conc)+(1|instrlab)+(1|Sequence)"),x,control = lme4::lmerControl()), error=function(e){
  return(NA)
}))})
```
...
```{r eval=FALSE}
getProteins(proteinsCPTAC)[na_indices]
```
...
```{r eval=FALSE}
lm_normal <- test.protLMcontrast(modelLmCPTAC, L, squeezeVar=FALSE, min_df=1, robust_var=TRUE, simplify=TRUE)
...
```{r eval=FALSE}
lm_Sq <- test.protLMcontrast(modelLmCPTAC, L, squeezeVar=TRUE, min_df=1, robust_var=TRUE, simplify=TRUE)
...
```{r eval=FALSE}
lm_SqM <- test.protLMcontrast(modelLmMCPTAC, L, squeezeVar=TRUE, min_df=1, robust_var=TRUE, simplify=TRUE)
...
```{r eval=FALSE}
ridge <- test.protLMcontrast(modelRidgeCPTAC, L, squeezeVar=FALSE, min_df=1, robust_var=TRUE, simplify=TRUE)
...
```{r eval=FALSE}
ridgeSq <- test.protLMcontrast(modelRidgeCPTAC, L, squeezeVar=TRUE, min_df=1, robust_var=TRUE, simplify=TRUE)
...
```{r eval=FALSE}
ridgeSqM <- test.protLMcontrast(modelRRCPTAC, L, squeezeVar=TRUE, min_df=1, robust_var=TRUE, simplify=TRUE)
...
###5. Create the summary tables "summary_lm" and the like
...
```{r eval=FALSE}
pepHYE124Df <- importDIAData("https://github.com/IFIproteomics/LFQbench/raw/master/ext/data/vignette_examples/hye124/Spectronaut_TTOF6600_64w_example.tsv", aggr_by="sequence.var")
head(pepHYE124Df)
```

The resulting data frame is of class `tbl_df` (see package `tibble`). To convert it to an ordinary data frame, use the `as.data.frame` function.

```{r eval=FALSE}
pepHYE124Df <- as.data.frame(pepHYE124Df)
head(pepHYE124Df)
```

Note that the `importDIAData` function has automatically added a "species" column. This will come in handy when evaluating our testing for differential abundance between sample B and sample A.
...
```{r eval=FALSE}
exp_annotation <- data.frame(run=c("lgillet_I150211_008", "lgillet_I150211_010", "lgillet_I150211_012", "lgillet_I150211_009", "lgillet_I150211_011", "lgillet_I150211_013"), sample=c("A","A","A","B","B","B"))
exp_annotation
```


###2.2. Preprocess the data using the preprocess_long function.
...
```{r eval=FALSE}
pepHYE124Df2QN <- preprocess_long(pepHYE124Df, split="/", accession="EG.ProteinId", exp_annotation=exp_annotation, type_annot=NULL, intensity_col="quant_value", run_col="R.FileName", aggr_by="EG.StrippedSequence", aggr_function="sum", logtransform=TRUE, base=2, normalisation="quantiles", smallestUniqueGroups=TRUE, useful_properties=c("species"), filter=NULL, filter_symbol="True", minIdentified=2, colClasses_df=NA, colClasses_exp=NA)
```
...
```{r eval=FALSE}
pepHYE124Df2 <- preprocess_long(pepHYE124Df, split="/", accession="EG.ProteinId", exp_annotation=exp_annotation, type_annot=NULL, intensity_col="quant_value", run_col="R.FileName", aggr_by="EG.StrippedSequence", aggr_function="sum", logtransform=TRUE, base=2, normalisation="none", smallestUniqueGroups=TRUE, useful_properties=c("species"), filter=NULL, filter_symbol="True", minIdentified=2, colClasses_df=NA, colClasses_exp=NA)
```
...
```{r eval=FALSE}
proteinsHYE124QN <- df2protdata(pepHYE124Df2QN, acc_col="EG.ProteinId", int_cols="quant_value", int_name="quant_value", annotations=c("species"), run_name="R.FileName")

proteinsHYE124 <- df2protdata(pepHYE124Df2, acc_col="EG.ProteinId", int_cols="quant_value", int_name="quant_value", annotations=c("species"), run_name="R.FileName")
```
...
```{r eval=FALSE}
resultHYE124QN <- test.protLMcontrast(protLMHYE124QN, L, min_df=1)
resultHYE124QN <- prot.p.adjust(resultHYE124QN, method="fdr")
resultHYE124QN <- prot.signif(resultHYE124QN, level = 0.05)

resultHYE124 <- test.protLMcontrast(protLMHYE124, L, min_df=1)
