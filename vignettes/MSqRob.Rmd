...
Note that the latest version of MSqRob can always be found at: https://github.com/ludgergoeminne/MSqRob.

The latest version of this vignette can be found at: https://github.com/ludgergoeminne/MSqRob/blob/master/vignettes/MSqRob.Rmd.
...
Additionally, we load the package `Biobase`, as we will use a few of its functions in this vignette.
...
```{r eval=FALSE}
devtools::install_github("ludgergoeminne/MSqRobData")
library(MSqRobData)
```
...
###4.2 Compare your results to ours

The MsqRobData package also contains the file "project_Francisella_models.RDatas", in which the `proteinsFranc` and the `protLMFranc` objects are saved. These should be identical to your results. In order to load this file, first specify the folder where it is saved.

```{r eval=FALSE}
result_path <- system.file("extdata/Francisella", "project_Francisella_models.RDatas", package = "MSqRobData")
```
...
Only "proteins" and "models" are of interest to us. The other objects are settings that are saved automatically when using the MSqRob Shiny App. We now load the file and compare the saved results to our results.

```{r eval=FALSE}
#Load the "project_Francisella_models.RDatas" file
...
The other elements in the list are only used within the MSqRob Shiny app.

```{r eval=FALSE}
result_files$levelOptions #Contains all contrast options for the fixed effects (see also 5. Statistical inferece)
result_files$random #Contains all random effects
result_files$plot2DependentVars #Contains a list with all fixed and random effects
```

You can save your proteins and models to a .RDatas object that can be loaded in R or in the Shiny graphical user interface. If you want your .RDatas object to be loaded into the Shiny App, you should also specify the `levelOptions`, `random` and `plot2DependentVars` options. In this example, I save the .RDatas object to my desktop.
...
saves_MSqRob(result_files, file="/Users/lgoeminn/Desktop/models_Francisella_new.RDatas") #Change file path to your desired save location!
```
...
The MsqRobData package contains the `modelCPTAC_RR` and the `proteinsCPTAC` objects in the "project_CPTAC_models.RDatas" file. These should be identical to your results.

```{r eval=FALSE}

result_path <- system.file("extdata/CPTAC", "project_CPTAC_models.RDatas", package = "MSqRobData")
result_files <- loads_MSqRob(result_path)

proteinsCPTAC_result <- result_files$proteins
modelCPTAC_RR_result <- result_files$models

proteinsCPTAC
proteinsCPTAC_result

modelCPTAC_RR
modelCPTAC_RR_result
```
...
```{r eval=FALSE}
L <- makeContrast(c("condition6B-condition6A","condition6C-condition6A","condition6D-condition6A","condition6E-condition6A","condition6C-condition6B","condition6D-condition6B","condition6E-condition6B","condition6D-condition6C","condition6E-condition6C","condition6E-condition6D"),
                  c("condition6A","condition6B","condition6C","condition6D","condition6E"))
```
...
lm_Sq <- test.protLMcontrast(mmodelCPTAC_Lm_Sq, L)
...
ridgeSqM_custom <- test.protLMcontrast(modelRRCPTAC, L, squeezeVar=TRUE, robust_var=TRUE, simplify=TRUE, custom_dfs = custom_dfs)
...
## HYE124 dataset (data-independent, label-free)

In order to easily import data-independent datasets, we suggest installing the `LFQbench` package. Information on how to install this package can be found at: https://github.com/IFIproteomics/LFQbench.
...
This example shows how to analyze label-free SWATH-MS data. SWATH-MS is a data-independent acquisition method using sequential window acquisition of all theoretical fragment-ion spectra (Gilet et al., 2012). We make use of the data provided by Navarro et al. (2016). In their study, the authors compare the performance of the five most popular software methods for searching (SWATH)-MS data: OpenSWATH, SWATH 2.0, Skyline, Spectronaut and DIA-Umpire using their R package `LFQbench`.

Briefly, the authors combined tryptic peptides from human HeLa cells, the yeast *Saccharomyces cerevisiae bayanus* strain Lalvin EC-1118 and a tryptic digest of *Escherichia coli* proteins (MassPREP standard, Waters Corporation) in the following ratios:

 - **sample A:** 65% w/w human, 30% w/w yeast, and 5% w/w *E. coli* proteins

 - **sample B:** 65% w/w human, 15% w/w yeast, and 20% w/w *E. coli* proteins

Both samples were run in technical triplicate on both a TripleTOF 5600 system (ABSciex) and a TripleTOF 6600 system (ABSciex) in both a 32-fixed-window setup and a 64-variable-window setup.
In this experiment, we will refer to A and B as "samples". In total, there are 6 mass spec runs.

All data files can be downloaded at: http://www.ebi.ac.uk/pride/archive/projects/PXD002952/files.

###1. Importing data

To import data from DIAumpire, OpenSWATH, PeakView, Skyline or Spectronaut, we make use of the `importDIAData function`. This function is largely based on the `FSWE.addSoftwareConfiguration` function from the `LFQbench` package (Navarro et al., 2016).

In this example, we will analyse a Spectronaut example file from the `LFQbench` vignette. This file can be directly downloaded from GitHub and imported into R.

```{r eval=FALSE}
pepHYE124Df <- importDIAData(experimentFile="https://github.com/IFIproteomics/LFQbench/raw/master/ext/data/vignette_examples/hye124/Spectronaut_TTOF6600_64w_example.tsv", aggr_by="sequence.var")
head(pepHYE124Df)

#!!!!Uitwerken voorbeeld xlsx file met wide!!!!!:

test <- importDIAData(experimentFile="http://www.ebi.ac.uk/pride/data/archive/2016/09/PXD002952/PViewNoFilter_TTOF5600_64w_10min_50ppm.xlsx", aggr_by="sequence.var")
head(pepHYE124Df)
```
...
We again create an experimental annotation data frame. Make sure not to mix up mass spec run annotations (mass spec run 08, 10 and 12 correspond to sample A, while run 09, 11 and 13 correspond to sample B!).

```{r eval=FALSE}
exp_annotation <- data.frame(run=c("lgillet_I150211_008", "lgillet_I150211_009", "lgillet_I150211_010", "lgillet_I150211_011", "lgillet_I150211_012", "lgillet_I150211_013"), sample=c("A","B","A","B","A","B"))

exp_annotation
```

###2.2. Add a species column

In the HYE124 dataset, we know that yeast proteins are more abundant in sample A, *E. coli* proteins are more abundant in sample B and human proteins are equally abundant in both samples. We will add a species indicator, which will help us later on in evaluating our method.

```{r eval=FALSE}
#Make a function to guess the species (adapted from the guessOrganism function from the LFQ bench package)
guessOrganism <- function (proteinid, species) 
{
  sp <- names(which(sapply(species, grepl, proteinid)))
  sp[sp=="YEAS8"] <- "YEAST"
    if (length(unique(sp)) == 0) 
        sp <- "NA"
...
pepHYE124Df$species <- vapply(pepHYE124Df$EG.ProteinId, guessOrganism, "",  c("HUMAN","YEAST","YEAS8","ECOLI"))

```

###2.3. Preprocess the data using the preprocess_long function.

Note that data frame `pepHYE124Df` is in "long format" (i.e. the data frame has one observation row per measurement (thus, multiple rows per peptide sequence, one for each sample in which that peptide sequence is identified). We will thus use the `preprocess_long` function.

The `run_col` argument indicates which column in data frame `pepHYE124Df` contains the indicators for the different mass spec runs. The other arguments are similar to the `preprocess_wide` function.

```{r eval=FALSE}
pepHYE124Df2QN <- preprocess_long(pepHYE124Df, split="/", accession="EG.ProteinId", exp_annotation=exp_annotation, type_annot=NULL, quant_col="quant_value", run_col="R.FileName", aggr_by="EG.StrippedSequence", aggr_function="sum", logtransform=TRUE, base=2, normalisation="quantiles", smallestUniqueGroups=TRUE, useful_properties=c("species"), filter=NULL, filter_symbol="True", minIdentified=2, colClasses_df=NA, colClasses_exp=NA)
```

We will also make a data frame `pepHYE124Df2` that will undergo the same preprocessing, except for the quantile normalisation (`normalisation="none"`). We will further show that this approach is more performant for this particular dataset (i.e. a dataset where there are a lot of differentially abundant proteins).

```{r eval=FALSE}
pepHYE124Df2 <- preprocess_long(pepHYE124Df, split="/", accession="EG.ProteinId", exp_annotation=exp_annotation, type_annot=NULL, quant_col="quant_value", run_col="R.FileName", aggr_by="EG.StrippedSequence", aggr_function="sum", logtransform=TRUE, base=2, normalisation="none", smallestUniqueGroups=TRUE, useful_properties=c("species"), filter=NULL, filter_symbol="True", minIdentified=2, colClasses_df=NA, colClasses_exp=NA)
```

We can make density plots for the overall log2-intensities of the different mass spec runs. Note that the technical variation appears to be very small and that the major difference in intensities is present between sample A and sample B.

```{r eval=FALSE}
plot(density(log2(subset(pepHYE124Df, pepHYE124Df$R.FileName==c("lgillet_I150211_008"))$quant_value)), main="Density before normalisation", las=1, bty="n")
lines(density(log2(subset(pepHYE124Df, pepHYE124Df$R.FileName==c("lgillet_I150211_010"))$quant_value)))
lines(density(log2(subset(pepHYE124Df, pepHYE124Df$R.FileName==c("lgillet_I150211_012"))$quant_value)))

lines(density(log2(subset(pepHYE124Df, pepHYE124Df$R.FileName==c("lgillet_I150211_009"))$quant_value)), col="red")
lines(density(log2(subset(pepHYE124Df, pepHYE124Df$R.FileName==c("lgillet_I150211_011"))$quant_value)), col="red")
lines(density(log2(subset(pepHYE124Df, pepHYE124Df$R.FileName==c("lgillet_I150211_013"))$quant_value)), col="red")
legend(x=1.2, y = 0.2, c("sample A","sample B"), col=c("black","red"), bty="n", lwd=1)
```

After quantile normalisation, all distributions have the same shape. Note that for plotting the densities after preprocessing, we don't put a log2-transformation in our code below, as log2-transformation has already been done during preprocessing.

```{r eval=FALSE}
plot(density(subset(pepHYE124Df2QN, pepHYE124Df2QN$R.FileName==c("lgillet_I150211_008"))$quant_value), main="Density after quantile normalisation", las=1, bty="n")
lines(density(subset(pepHYE124Df2QN, pepHYE124Df2QN$R.FileName==c("lgillet_I150211_010"))$quant_value))
lines(density(subset(pepHYE124Df2QN, pepHYE124Df2QN$R.FileName==c("lgillet_I150211_012"))$quant_value))

lines(density(subset(pepHYE124Df2QN, pepHYE124Df2QN$R.FileName==c("lgillet_I150211_009"))$quant_value), col="red")
lines(density(subset(pepHYE124Df2QN, pepHYE124Df2QN$R.FileName==c("lgillet_I150211_011"))$quant_value), col="red")
lines(density(subset(pepHYE124Df2QN, pepHYE124Df2QN$R.FileName==c("lgillet_I150211_013"))$quant_value), col="red")
legend(1.2, y = 0.2, c("sample A","sample B"), col=c("black","red"), bty="n", lwd=1)
```


###3. Convert the data frame into a protdata object.

We will evaluate both the unnormalized and the quantile normalized data.

```{r eval=FALSE}
proteinsHYE124QN <- df2protdata(pepHYE124Df2QN, acc_col="EG.ProteinId", quant_cols="quant_value", quant_name="quant_value", annotations=c("species"), run_name="R.FileName")

proteinsHYE124 <- df2protdata(pepHYE124Df2, acc_col="EG.ProteinId", quant_cols="quant_value", quant_name="quant_value", annotations=c("species"), run_name="R.FileName")
```

###4. Fit the model and borrow information over variances

```{r eval=FALSE}
protLMHYE124QN <- fit.model(proteinsHYE124QN, response="quant_value", fixed=c("sample"),  random=c("EG.StrippedSequence","R.FileName"), add.intercept=TRUE, weights="Huber")
protLMHYE124QN <- squeezePars(protLMHYE124QN)

protLMHYE124 <- fit.model(proteinsHYE124, response="quant_value", fixed=c("sample"),  random=c("EG.StrippedSequence","R.FileName"), add.intercept=TRUE, weights="Huber")
protLMHYE124 <- squeezePars(protLMHYE124)
```

###4.1. Inspect the model

```{r eval=FALSE}
attr(getModels(protLMHYE124QN[1]),"MSqRob_levels")
attr(getModels(protLMHYE124[1]),"MSqRob_levels")
```

###5. Statistical inference

Make the contrast matrix `L`.

```{r eval=FALSE}
L <- makeContrast(contrasts=c("sampleB-sampleA"),
                  levels=c("sampleA","sampleB"))
L
```

Test the contrast and calculate FDR values.

```{r eval=FALSE}
resultHYE124QN <- test.protLMcontrast(protLMHYE124QN, L, par_squeeze = c("EG.StrippedSequence","R.FileName"))
resultHYE124QN <- prot.p.adjust(resultHYE124QN, method="fdr")
resultHYE124QN <- prot.signif(resultHYE124QN, level = 0.05)

resultHYE124 <- test.protLMcontrast(protLMHYE124, L, par_squeeze = c("EG.StrippedSequence","R.FileName"))
resultHYE124 <- prot.p.adjust(resultHYE124, method="fdr")
resultHYE124 <- prot.signif(resultHYE124, level = 0.05)
```
...
Test how many proteins are falsely denoted as being differentially abundant (remember: human proteins are not differentially abundant, yeast proteins are higher abundant in sample A and *E. coli* proteins are higher abundant in sample B).

```{r eval=FALSE}
sign_species <- subset(resultHYE124QN, resultHYE124QN$signif==TRUE)$species
length(sign_species)
sum(sign_species=="HUMAN")
#FDR:
sum(sign_species=="HUMAN")/(sum(sign_species=="HUMAN")+length(sign_species))
#0.24

sign_species <- subset(resultHYE124, resultHYE124$signif==TRUE)$species
length(sign_species)
sum(sign_species=="HUMAN")
#FDR:
sum(sign_species=="HUMAN")/(sum(sign_species=="HUMAN")+length(sign_species))
...
Note that although we put our FDR level at 5%, the FDR is not controlled at this level at all (9% if no normalisation in applied and 24% with quantile normalisation). This is because of the nature of the dataset: we analyzed the data as if each mass spec run was an independent sample. This is of course not true. In reality, these runs just differ in technical variability. A better spike-in experiment would have been to repeat the spiking 3 times, each time deliberately making a small error in order to better mimick real biological variability.

We want to compare the performance of our workflow with and without quantile normalisation. Therefore, we will make ROC (Receiver Operating Characteristic) curves.

```{r eval=FALSE}
species <- resultHYE124QN$species

TP <- c(0,cumsum(species!="HUMAN"))/sum(species!="HUMAN")#sum(zlmall[,9])
FP <- c(0,cumsum(species=="HUMAN"))/sum(species=="HUMAN")#length(zlmall[,9])
plot(TP~FP, type="l", xlim=c(0,1), ylim=c(0,1), xlab="% false positives", ylab="% true positives", col="blue", las=1, bty="n")

species <- resultHYE124$species

TP <- c(0,cumsum(species!="HUMAN"))/sum(species!="HUMAN")#sum(zlmall[,9])
FP <- c(0,cumsum(species=="HUMAN"))/sum(species=="HUMAN")#length(zlmall[,9])
lines(TP~FP, type="l", xlim=c(0,1), ylim=c(0,1), xlab="% false positives", ylab="% true positives", col="green", las=1, bty="n")
legend(0.4, y = 0.4, c("No normalisation","Quantile normalisation"), col=c("green","blue"), bty="n", lwd=1)
```

How is it possible that no normalisation performs better than quantile normalisation in this case? This is because the number of proteins that is differentially abundant (i.e. yeast and *E. coli* proteins) is very large (357 proteins) compared to the non-differentially abundant human proteins (only 302 proteins!). Most normalisation approaches, including quantile normalisation, assume that the vast majority of proteins is not significantly differentially abundant. If this is the case, their peptide distributions can be "forced" onto a similar distribution, thereby reducing technical en biological variability. However, if a non-neglegible part of the proteins is differentially abundant, this assumtion is violated and an analysis on normalised data can indeed perform worse than non-normalised data, especially in this case, where there is no biological variability and technical variability is very small. In real biological datasets, normalisation is typically advised, unless there are good reasons not to do so.
